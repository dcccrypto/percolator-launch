...
//             = 368 bytes
// NOTE: PERC-298 skew_factor is packed into upper bits of oi_cap_multiplier_bps — no struct growth.
...
// =============================================================================
// RiskEngine Layout Constants (updated for PERC-298: long_oi, short_oi added to engine)
// ENGINE_OFF = align_up(HEADER_LEN + CONFIG_LEN, 8) = align_up(104 + 368, 8) = 472
//   (SBF: U128 = [u64;2] so RiskEngine alignment = 8, not 16)
//
...
//   circuit breaker, OI cap fields). No change in PERC-298 (skew packed into existing field).
// RiskEngine grew by 32 bytes (PERC-298: added long_oi, short_oi U128 fields after total_oi).
// =============================================================================
...
// PERC-298: Directional OI tracking (inserted after total_open_interest, shifting all below +32)
const ENGINE_LONG_OI_OFF = 432;         // U128: sum of long position sizes
const ENGINE_SHORT_OI_OFF = 448;        // U128: sum of abs(short position sizes)
...
// Bitmap starts at 608 (was 576, +32 from PERC-298 long_oi/short_oi)
...
// Large: engine fixed(608) + bitmap(512) + 18 + next_free(8192) = 9330, align to 8 = 9336
const ENGINE_ACCOUNTS_OFF = 9336;       // accounts offset for 4096 variant (within engine, +32 from PERC-298)

/**
 * Compute bitmap words and accounts offset for a given maxAccounts.
 * Layout within engine: fixed(608) + bitmap(words*8) + post_bitmap(18) + next_free(N*2) + padding + accounts(N*248)
...
export interface EngineState {
  ...
  longOi: bigint;            // PERC-298: Sum of long position sizes
  shortOi: bigint;           // PERC-298: Sum of abs(short position sizes)
