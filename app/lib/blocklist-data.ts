/**
 * Blocked slab addresses — THE canonical list, and the only place to edit.
 *
 * WHY THIS FILE IS SEPARATE (and must stay import-free)
 * -----------------------------------------------------
 * This module is PURE DATA: zero imports, zero side effects, no `process.env`.
 * That is load-bearing, not stylistic. `lib/blocklist.ts` layers env-var
 * overrides on top of this array, which makes it Node-flavoured, and it is
 * imported by many Node-runtime route handlers — so when the Edge middleware
 * imported it, Vercel's deploy-time validator rejected the build:
 *   The Edge Function "middleware" is referencing unsupported modules
 * The old fix was a hand-maintained SECOND copy of every address in
 * `lib/blocklist-edge.ts`, kept in sync by a test. This file replaces that
 * duplication: both consumers import this one array, and the edge bundle
 * stays clean because there is nothing here to make it dirty.
 *
 * If you add anything to this file that imports or touches `process`, the
 * edge build breaks again — put it in `lib/blocklist.ts` instead.
 */
export const HARDCODED_BLOCKED_SLABS: readonly string[] = [
